...
def HOME():
	addDir('THAI','http://www.merlkon.com/albumcategory/thai-videos/',2,'http://khmernz.com/xbmc/khmer2all/thai.png')
	addDir('KOREAN','http://www.merlkon.com/albumcategory/korean-videos/',2,'http://khmernz.com/xbmc/khmer2all/korean.png')
	addDir('CHINESE','http://www.khmerave.com/albumcategory/chinese-videos/',2,'http://khmernz.com/xbmc/khmer2all/chinese.png')
	addDir('CHINESE VIDEO2','http://www.merlkon.com/albumcategory/chinese-videos/',2,'http://khmernz.com/xbmc/khmer2all/chinese.png')
	addDir('Bollywood','http://www.merlkon.com/albumcategory/bollywood-videos/',2,'http://khmernz.com/xbmc/khmer2all/bollywood.png')
	addDir('Philippines','http://www.merlkon.com/albumcategory/philippines-videos/',2,'http://khmernz.com/xbmc/khmer2all/philippine.png')
	addDir('KHMER','http://www.merlkon.com/albumcategory/khmer-media/',2,'http://khmernz.com/xbmc/khmer2all/khmer.png')
	addDir('KHMER VIDEO2','http://www.khmerave.com/albumcategory/khmer-media/',2,'http://khmernz.com/xbmc/khmer2all/khmer.png')	
def INDEX(url):
    #try:
        link = GetContent(url)
        try:
        ...
        match=re.compile('<div class="widget widget-1 widget-single clearfix widget_text enhanced-text-widget" id="enhancedtextwidget-17">(.+?)<div class="builder-module-block-outer-wrapper builder-module-sidebar-outer-wrapper builder-module-column-2-outer-wrapper right clearfix">').findall(newlink)
        match=re.compile('<a href="(.+?)" title="(.+?)"><img src="(.+?)"').findall(match[0])
        ...
        if(len(match5) >= 1 and len(match5[0]) >= 1 ):
                pagelist =re.compile('<a class="nextpostslink" href="([^>]+)">([^>]+)</a>').findall(match5[0][0])
                for pageurl,pagenum in pagelist:
					addDir("Page " + pagenum.replace('\xc2\xab',"Prev <<").replace('\xc2\xbb',"Next >>"),pageurl.encode("utf-8"),2,"")
    #except: pass
...
def Episodes(url,name):
    #try:
        ...
        match=re.compile('<div style="padding-left:8px">(.+?)<div id="comments">').findall(newlink)
        match=re.compile('<a href="(.+?)"><span style=".+?">(.+?)</span></a>').findall(match[0])
        counter = 1
        videolist =url+";#"
        vidPerGroup = 5
        if(len(match) >= 1):
