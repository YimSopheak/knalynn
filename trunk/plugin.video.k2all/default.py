...
strDomain ='http://www.khmer2all.com'
...
def HOME():
        ...
        addDir('Sokea Leakhena BIG','http://www.khmer2all.com/browse-sokealeakhenabigshow-videos-1-date.html',2,'http://www.khmernz.com/xbmc/khmer2all/sokea.png')
...
def Episodes(url,name):
    try:
        link = GetContent(url)
        newlink = ''.join(link.splitlines()).replace('\t','')
        match=re.compile('<div id="browzzse_results" (.+?)<ul>(.+?)</ul></div>').findall(newlink)
        if(len(match) >= 1):
