...
def LIST_ITEMS(url):
  html = GET_HTTP(url)
  match=re.compile('<a href="(.+?)"><img src="(.+?.jpg)" alt=".+?" title="(.+?)" /></a>').findall(html)
  next=re.compile('class="current">.+?</a>.+?<a href="(.+?)" target="_self">').findall(html)

  for url,image,title in match:
    addDownLink(title,url,3,image)
  for next_page in next:
    addDir(' Next Page',next_page,2,'')
